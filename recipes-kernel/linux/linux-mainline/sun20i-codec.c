...
#define SUN20I_CODEC_POWER_BG_TRIM_MASK		GENMASK(7, 0)
/* Voltage selector 3 => 1.8V (range 1.65V..2.0V, 50mV steps). */
#define SUN20I_CODEC_POWER_LDO_1V8_SEL		3
/* Documented default bandgap trim (~900mV) when no eFuse value is applied. */
#define SUN20I_CODEC_POWER_BG_TRIM_DEFAULT	0x19
...
static int sun20i_codec_component_probe(struct snd_soc_component *component)
{
	...
	/*
	 * Power the on-die analog LDOs directly. Upstream models these as the
	 * "avcc"/"hpvcc" regulators of a separate sun20i-analog-ldos driver, but
	 * that driver is absent from mainline 6.5.5 and its eFuse "bg_trim" cell
	 * is a bitfield that nvmem_cell_read_u8() rejects (-EINVAL) - which left
	 * the codec deferring forever and its reset stuck (-EBUSY). Drive the
	 * POWER register ourselves so the card probes standalone: apply the
	 * default bandgap trim, select 1.8V for both LDOs, then enable ALDO
	 * (avcc, feeds DAC/ADC) and HPLDO (hpvcc, feeds the headphone driver).
	 * This register lives in an always-on domain (see note above).
	 */
	snd_soc_component_update_bits(component, SUN20I_CODEC_POWER,
				      SUN20I_CODEC_POWER_BG_TRIM_MASK,
				      SUN20I_CODEC_POWER_BG_TRIM_DEFAULT);
	snd_soc_component_update_bits(component, SUN20I_CODEC_POWER,
				      SUN20I_CODEC_POWER_ALDO_VOLTAGE_MASK |
				      SUN20I_CODEC_POWER_HPLDO_VOLTAGE_MASK,
				      (SUN20I_CODEC_POWER_LDO_1V8_SEL << 12) |
				      (SUN20I_CODEC_POWER_LDO_1V8_SEL << 8));
	snd_soc_component_update_bits(component, SUN20I_CODEC_POWER,
				      SUN20I_CODEC_POWER_ALDO_EN_MASK |
				      SUN20I_CODEC_POWER_HPLDO_EN_MASK,
				      SUN20I_CODEC_POWER_ALDO_EN_MASK |
				      SUN20I_CODEC_POWER_HPLDO_EN_MASK);
